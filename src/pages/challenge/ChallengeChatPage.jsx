...
const ChallengeChatPage = () => {
    ...
    const ChatLoading = () => {
      ...
      .then(datas => { 
          ...
          stompClient.send('/pub/chat/'+RoomType, {}, JSON.stringify({ 'chatMessage' : nickname + '가(이) 입장했어요', 'challengeNick': nickname,'chatroomId': chatroom,'messageType':'ENTER','userId':userId}));
      })
      .catch(error => {
    ...
    useEffect(() => {
      ...
      stompClient.connect({}, () => {
          ...
          const handleUnload = (ev) => {
            ev.preventDefault();
            stompClient.send('/pub/chat/'+RoomType, {}, JSON.stringify({ 'chatMessage' : nickname + '가(이) 퇴장했습니다.', 'challengeNick': nickname,'chatroomId': chatroom,'messageType':'LEAVE','userId':userId}));
    ...
    const handleSend = (message) => {
      stompClient.send('/pub/chat/'+RoomType,{},JSON.stringify({ 'chatMessage' : message,'challengeNick': nickname,'chatroomId': chatroom,'messageType':'CHAT','userId':userId}));
    };


    const handleSendModal = (inputValue) => {
      console.log(inputValue)
      fetch(`http://localhost:9999/challenge/account`, {method:'POST',body:JSON.stringify({ 'chatroomId' : chatroom,'challengeNick': inputValue,'userId': 3}),
              headers:{'content-type':'application/json'}})//@RequestBody로 받는다
      .then(response=>response.json())
      .then(data=>{       
          if(data.memberOk==1) {
              userId = data.userId;
    ...
    return (
        <PageContainer>
            <ChallengeChatContainer>
                ...
                <ChatUI title="Challenge Chat" overflow={true} height="300px" data={chatData} onSend={handleSend} nickname={nickname} />
