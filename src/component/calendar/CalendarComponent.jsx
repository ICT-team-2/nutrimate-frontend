        ...
        if (response.data.length !== 0) {
          for (const data in response.data) {
            ...
            for (const key in response.data[data]) {
              ...
              transformedItem[newKey] = key === 'alarmCategory' ? (<>{<CircleIcon style={{
                color: '#0f4a15',
                fontSize: '8px',
                margin: '0 2px',
    ...
    <>
      <CalendarContainer>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <Button style={{
            padding: '8px',
            boxShadow: isClickCalendar ? '0px 4px 8px rgba(0, 0, 0, 0.2)' : 'none',
            backgroundColor: isClickCalendar ? '#007500' : '',
          }} onClick={callendar}>
            <CalendarMonthIcon style={{ fontSize: '32px', color: isClickCalendar ? 'white' : '' }} />
          </Button>
          <Button onClick={list} style={{
            padding: '8px',
            boxShadow: isClickList ? '0px 4px 8px rgba(0, 0, 0, 0.2)' : 'none',
            backgroundColor: isClickList ? '#007500' : '',
          }}>
        ...
        >
          <MenuItem style={{
            backgroundColor: '#0f4a15',
            color: 'white',
          }}>{dayjs(anchorEl?.day).format('YYYY-MM-DD')}</MenuItem>
          {anchorEl?.item.map((item, index) => (
            <MenuItem style={{ fontSize: '15px' }} key={index}> <span
              style={{ marginRight: '10px', fontSize: '12px', fontWeight: 'bold' }}>{item.title}</span><span
              style={{ fontSize: '10px' }}>{dayjs(item.start).format('A hh시 mm분 ')}</span></MenuItem>
          ))}
        </Menu>
      </CalendarContainer>
    </>
