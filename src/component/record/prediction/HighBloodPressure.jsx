...
const HighBloodPressurePrediction = ({ surveyInput }) => {
  ...
  const [chol] = useState(surveyInput.chol || '');
  ...
  const fetchPrediction = async () => {
    ...
    try {
      ...
      const cpValue = cp === 2 ? 0 : 1;
      const restecgValue = restecg === 2 ? 0 : 1;
      const exangValue = exang === 2 ? 0 : 1;
      const oldpeakValue = oldpeak === 2 ? 1.0 : 3.0;
      const slopeValue = slope === 2 ? 0 : 2;
      const caValue = ca === 2 ? 0 : 2;
      const thalValue = thal === 2 ? 1 : 3;

      const response = await axios.get(
        `${import.meta.env.REACT_APP_FLASK_URL}/hypertension?sex=${genderValue}&age=${age}&fbs=${glucoseValue}&trestbps=${bloodPressure}`
        + `&cp=${cpValue}&chol=${chol}&restecg=${restecgValue}&thalach=${thalach}&exang=${exangValue}&oldpeak=${oldpeakValue}&slope=${slopeValue}&ca=${caValue}&thal=${thalValue}`,
      );
      const { no_hypertension_proba, hypertension_proba } = response.data;
      setHypertensionResult({
        prediction: {
          'no_hypertension_proba': no_hypertension_proba,
          'hypertension_proba': hypertension_proba,
        },
      });
    } catch (error) {
      ...
      if (prediction <= 30) {
        return (
          <Alert severity="success">
            ...
            <br /> 식습관 면에서는, 신선한 과일과 채소를 많이 섭취하고, 전곡류와 저지방 단백질을 꾸준히 먹는 것이 좋아요.
            소금과 설탕 섭취는 계속해서 적게 유지하고, 가공식품과 패스트푸드는 가급적 피하세요.
            운동은 주 150분의 중등도 신체 활동 또는 75분의 고강도 신체 활동, 또는 이 둘의 조합을 목표로 하세요. 스트레칭이나 요가 같은 활동을 통해 유연성도 증진시키고,
            <br /> 마음의 안정을 찾는 시간도 가지는 것이 좋습니다. 수면은 매일 밤 최소 7-9시간을 목표로 하고, 규칙적인 수면 습관을 유지하는 것이 중요해요.
            스트레스는 고혈압에 큰 영향을 줄 수 있으므로, 스트레스를 관리하는 방법을 찾고, 긍정적인 사고방식을 유지하는 것도 중요합니다.
            <br /> 현재의 건강한 생활 습관을 유지하면서, 끊임없이 자신을 돌보고, 건강 상태를 주기적으로 체크하는 것이 중요합니다. 당신의 건강한 라이프스타일이 앞으로도 계속해서 당신을 지켜줄
            것입니다.
          </Alert>
        );
        ...
        return (
          <Alert severity="warning">
            ...
            <br /> 또한, 정기적인 건강 검진과 전문가의 조언을 구하는 것도 중요합니다. 혹시 이미 고혈압이나 다른 건강 문제가 있다면, 의사의 지시에 따라 약물 치료를 병행하는 것도 고려해야 할 수
            있습니다.
            중요한 것은, 지금부터라도 건강한 생활 습관을 통해 고혈압의 위험을 줄이고, 더 건강한 삶을 위해 노력하는 것입니다.
            <br /> 자신의 건강을 챙기는 일이 결코 쉽지 않다는 것을 알고 있지만, 여러분이 할 수 있습니다. 지금 이 순간부터 고혈압 예방을 위해 노력하세요.
        ...
        return (
          <Alert severity="error">
            ...
            <br /> 이제부터 시작하는 건강한 식습관이 당신의 삶에 큰 변화를 가져올 수 있어요. 일상에서 소금 섭취를 줄이고, 가공식품과 패스트푸드는 가능한 피하도록 해보세요.
            <br /> 신선한 과일과 채소를 충분히 섭취하고, 전곡류, 견과류와 같은 심장에 좋은 음식을 식단에 포함시키는 것이 중요합니다.
            <br /> 또한, 규칙적인 식사 시간을 지키고, 과식을 피하는 것도 고혈압 관리에 도움이 됩니다.
            이 모든 변화가 처음에는 어렵게 느껴질 수 있지만, 건강을 위한 작은 선택이 모여 큰 결과를 만들어낼 수 있습니다.
            <br /> 당신의 건강을 위한 노력이 결국 더 행복하고 활기찬 삶으로 이끌 것입니다. 적극적인 관리를 위해 노력하세요.
          </Alert>
        );
      }
    }
  ...
  return (
    <div>
      {(
        <Stack sx={{ width: '100%', marginTop: '30px', marginBottom: '30px', marginRight: '10px', marginLeft: '10px' }}
               spacing={2}>
