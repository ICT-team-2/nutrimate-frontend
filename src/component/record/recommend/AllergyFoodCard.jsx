  ...
  return (
    <div style={{ marginTop: '10px', marginLeft: '10px', display: 'flex', flexDirection: 'row' }}>
                  ...
                  {Object.entries(diet.영양소정보)
                    ...
                    .map(([key, value]) => {
                      ...
                      return (
                        <span key={key}>
                                                <CircleIcon
                                                  style={{ color: color, fontSize: '10px', marginRight: '3px' }} />
      ...
      <Card style={{
        ...
        {userAllergy && userAllergy?.split(',').map((allergy, index) => (
          <Chip key={index} label={allergy.trim()}
                style={{ background: '#B5C4A4', color: 'white', marginRight: '10px', marginBottom: '5px' }} />
        ))}
      </Card>
    </div>
  );
